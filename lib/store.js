...
var constants_2 = require('redux-persist');
...
exports.DEFAULT_KEY_PREFIX = redux_persist_1.KEY_PREFIX;
var newStore = function(_a) {
	var _b = _a.clientGetter,
		clientGetter =
			_b === void 0
				? function() {
						return null;
		...
		keyPrefix = _a.keyPrefix,
		storage = _a.storage,
		_d = _a.callback,
		callback = _d === void 0 ? function() {} : _d;
	...
	var store = redux_1.createStore(
		...
		redux_1.compose(
			...
			redux_offline_1.offline(
				__assign(__assign({}, defaults_1.default), {
					retry: retry_link_1.getEffectDelay,
					persistCallback: function() {
						logger('Storage ready');
						persistCallback();
					},
					persistOptions: __assign(
						__assign(
							__assign({}, keyPrefix && { keyPrefix: keyPrefix + ':' }),
					...
					effect: function(effectPayload, action) {
						return effect(effectPayload, action, store, clientGetter(), callback, detectNetwork);
					},
...
var offlineEffectsConfigs = [ offline_link_1.offlineEffectConfig, deltaSync_1.offlineEffectConfig ]
	.filter(Boolean)
	.reduce(function(acc, _a) {
		var enqueueAction = _a.enqueueAction,
			rest = __rest(_a, [ 'enqueueAction' ]);
...
var reducer = function(dataIdFromObject) {
	...
	return (
		(_a = {}),
		(_a[index_1.METADATA_KEY] = function(state, action) {
			return Object.entries(offlineEffectsConfigs).reduce(function(acc, _a) {
				var _b = _a[1].reducer,
					reducer =
						_b === void 0
							? function() {
...
var effect = function(effect, action, store, clientGetter, callback, offlineStatusChangeCallbackCreator) {
